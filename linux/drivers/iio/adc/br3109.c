/*
 * BR3109 RF Transceiver
 *
 * Copyright 2018-2019 BriRadio Inc.
 *
...
int br3109_spi_read_word(struct spi_device *spi, unsigned reg)
{
...
int br3109_spi_write_word(struct spi_device *spi, unsigned reg, unsigned val)
{
	unsigned char buf[12];
	int ret;
...
static int br3109_do_setup(struct br3109_rf_phy *phy)
{
	...
	if (has_tx_and_en(phy)) {
		...
		/* RESET CDR */
		phy_ctrl = br3109_spi_read(phy->spi, TALISE_ADDR_DES_PHY_GENERAL_CTL_1);
		br3109_spi_write(phy->spi, TALISE_ADDR_DES_PHY_GENERAL_CTL_1, phy_ctrl & ~BIT(7));
		br3109_spi_write(phy->spi, TALISE_ADDR_DES_PHY_GENERAL_CTL_1, phy_ctrl);
	}
...
static struct attribute *br31081_phy_attributes[] = {
	&iio_dev_attr_ensm_mode.dev_attr.attr,
	&iio_dev_attr_ensm_mode_available.dev_attr.attr,
	&iio_dev_attr_multichip_sync.dev_attr.attr,
	&iio_dev_attr_calibrate.dev_attr.attr,
...
static const struct attribute_group br31081_phy_attribute_group = {
	.attrs = br31081_phy_attributes,
};

static struct attribute *br31082_phy_attributes[] = {
	&iio_dev_attr_ensm_mode.dev_attr.attr,
	&iio_dev_attr_ensm_mode_available.dev_attr.attr,
	&iio_dev_attr_multichip_sync.dev_attr.attr,
	&iio_dev_attr_calibrate.dev_attr.attr,
...
static const struct attribute_group br31082_phy_attribute_group = {
	.attrs = br31082_phy_attributes,
};
				   ...
				   u32 *readval)
{
	...
	mutex_lock(&indio_dev->mlock);
	if (readval == NULL)
		ret = br3109_spi_write_word(phy->spi, reg, writeval);
	else {
		*readval = br3109_spi_read_word(phy->spi, reg);
		ret = 0;
	}
	mutex_unlock(&indio_dev->mlock);
...
static const taliseObsRxLoSource_t br3109_obs_rx_port_lut[] = {
				    ...
				    const struct iio_chan_spec *chan, u32 mode)
{
	...
	ret = TALISE_setOrxLoSource(phy->talDevice, br3109_obs_rx_port_lut[mode]);
	if (!ret)
...
static const struct iio_chan_spec br31081_phy_chan[] = {
	{	/* RX LO */
		.type = IIO_ALTVOLTAGE,
		.indexed = 1,
		.output = 1,
...
static const struct iio_info br31081_phy_info = {
	.read_raw = &br3109_phy_read_raw,
	.write_raw = &br3109_phy_write_raw,
	.debugfs_reg_access = &br3109_phy_reg_access,
	.attrs = &br31081_phy_attribute_group,
	.driver_module = THIS_MODULE,
};

static const struct iio_chan_spec br31082_phy_chan[] = {
	{	/* TX LO */
		.type = IIO_ALTVOLTAGE,
		.indexed = 1,
		.output = 1,
...
static const struct iio_info br31082_phy_info = {
	.read_raw = &br3109_phy_read_raw,
	.write_raw = &br3109_phy_write_raw,
	.debugfs_reg_access = &br3109_phy_reg_access,
	.attrs = &br31082_phy_attribute_group,
	.driver_module = THIS_MODULE,
};
				      ...
				      const char __user *userbuf, size_t count, loff_t *ppos)
{
	...
	case DBGFS_BIST_FRAMER_B_LOOPBACK:
		mutex_lock(&phy->indio_dev->mlock);
		ret = br3109_spi_write(phy->spi, TALISE_ADDR_JESD_FRAMER_CFG4_0 +
...
static int br3109_probe(struct spi_device *spi)
{
	...
	clk = devm_clk_get(&spi->dev, (id == ID_BR31082) ?
	...
	if (id == ID_BR3109 || id == ID_BR3109_X2 || id == ID_BR31082)
	...
	case ID_BR3109_X2:
		...
		break;
	case ID_BR31081:
		indio_dev->info = &br31081_phy_info;
		indio_dev->channels = br31081_phy_chan;
		indio_dev->num_channels = ARRAY_SIZE(br31081_phy_chan);
		break;
	case ID_BR31082:
		indio_dev->info = &br31082_phy_info;
		indio_dev->channels = br31082_phy_chan;
		indio_dev->num_channels = ARRAY_SIZE(br31082_phy_chan);
		break;
	default:
		ret = -EINVAL;
...
MODULE_AUTHOR("Zheng Jianfeng <jianfeng.zheng@irelement.com>");
MODULE_DESCRIPTION("BriRadio BR3109 ADC");
