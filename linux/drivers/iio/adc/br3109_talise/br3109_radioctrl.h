/**
 * \file br3109_radioctrl.h
 * \brief Contains Br3109 related function prototypes for br3109_radioctrl.c
 *
...
#ifndef BR3109_RADIOCTRL_H_
#define BR3109_RADIOCTRL_H_
...
uint32_t BR3109_loadStreamFromBinary(br3109Device_t *device, uint8_t *binary);
 ...
 /**
  * \brief Instructs the Br3109 ARM which GPIO pins are used for TDD pin control
  *
  * The Br3109 can control any of these related signals by sending an ARM command, or
  * the Br3109 ARM signals can be routed to GPIO pins.  Each signal in the armGpio
  * structure has an enable member.  If the enable member is set, the Br3109 ARM
  ...
  * BR3109_loadArmFromBinary() function call.  If the BBP wishes to change the GPIO
  * assignments this function can be called again to change the configuration while
  * the ARM is in the radioOff state.  This function also sets the Br3109 GPIO pin
  * direction for any GPIO pins that are enabled in this function.
  *
  * \pre This function can be called after loading Br3109 ARM binary, but before
 ...
 *  
 * \param orxEnGpioPinSel Pointer to br3109GpioShortPinSel_t structure that
 * configures the orx enable pin control.
 *
 * \retval TALACT_WARN_RESET_LOG recovery action for log reset
 * \retval TALACT_ERR_CHECK_PARAM recovery action for bad parameter check
 * \retval TALACT_ERR_RESET_SPI recovery action for SPI reset required
 ...
 *
 *  If this feature is enabled, note that the Br3109 trackings calibrations
 *  will be limited or not usable, as some of the Br3109 tracking cals require
 *  using the AuxLO.
 *
 *  If this function is enabled to set the ORx LO source to AuxLO, the BBIC can
 *  control the ORx Lo source either using the BR3109_setOrxLoSource() function
 ...
 * \retval TALACT_NO_ACTION function completed successfully, no action required
 */
uint32_t BR3109_setOrxLoCfg(br3109Device_t *device, const br3109OrxLoCfg_t *orxLoCfg);
 ...
 *  BR3109_setOrxLoCfg() function.
 *
 * \pre This function can be called anytime after BR3109_setOrxLoCfg() has been
 ...
 * off-line state, call the BR3109_radioOff() function.
 *
 ...
 * keep the receive and transmit chains powered down until the BR3109_radioOn() function
 * is called again.
 *
 * \pre This function may be called any time BR3109_radioOn had been called previously
 *
 * \dep_begin
 * \dep{device->devHalInfo}
 * \dep_end
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t BR3109_setRxTxEnable(br3109Device_t *device, br3109RxORxChannels_t rxOrxChannel, br3109TxChannels_t txChannel);
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t BR3109_getRxTxEnable(br3109Device_t *device, br3109RxORxChannels_t *rxOrxChannel, br3109TxChannels_t *txChannel);
/**
 ...
 *
 * \param oRx1Map enumerated type br3109TxToOrxMapping_t which maps either Tx1 or Tx2 to ORx1.
 * \param oRx2Map enumerated type br3109TxToOrxMapping_t which maps either Tx1 or Tx2 to ORx2.
 *
 * Both oRx1Map and oRx2Map use the br3109TxToOrxMapping_t enumerated type encoded as a bit-field:
 ...
 * \retval TALACT_NO_ACTION Function completed successfully, no action required
 */
uint32_t BR3109_setTxToOrxMapping(br3109Device_t *device, uint8_t txCalEnable, br3109TxToOrxMapping_t oRx1Map, br3109TxToOrxMapping_t oRx2Map);
 ...
 * \param pllName Name of the PLL to configure based on the enumerated types in br3109RfPllName_t
 ...
 *
 * This function is used to get a PLL's current frequency. A br3109RfPllName_t enumerated type is passed for
 ...
 *
 * This function returns the status of the Br3109 PLLs via the pllLockStatus pointer.
 ...
 *
 * \deprecated This function is deprecated.  BR3109_setPllLoopFilter should be
 ...
 * BR3109_setRfPllFrequency function for the TAL_RF_PLL to set up the RF PLL
 ...
 *
 * \deprecated This function is deprecated.  BR3109_getPllLoopFilter should be
 ...
 * \pre This function can be called after the ARM has been initialized, and
 * device must be in radioOff state. This function must be followed with a
 * BR3109_setRfPllFrequency function for the selected PLL to set up the PLL
 ...
 *
 * \pre This function can be used after the device has been fully initialized
 *      and the ARM processor is initialized and BR3109_setFhmConfig() is called.
 ...
 *      BR3109_setFhmConfig() API and the FHM mode is set via
 *      BR3109_setFhmMode() API. This API can be called only in Radio ON state
 *
 * \dep_begin
 * \dep{device->devHalInfo}
 * \dep_end
 ...
 *
 * This function can be used to read the current FHM status. Please refer to br3109FhmStatus_t structure definition
 ...
 * BR3109_setRfPllFrequency() and the internal LO divider changes, it is likely this
