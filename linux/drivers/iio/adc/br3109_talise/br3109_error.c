/**
 * \file br3109_error.c
 * \brief Contains Br3109 API error information functions
...
static const char* talCallbackErrorMessage(br3109Device_t *device,
...
uint32_t TALISE_getErrCode(br3109Device_t *device, uint32_t* errSrc,
			   uint32_t* errCode)
{
...
const char* TALISE_getErrorMessage(uint32_t errSrc, uint32_t errCode)
{
	...
	if(errSrc == TAL_ERRSRC_BRHAL) {
		switch (errCode) {
		case BRHAL_OK:
			return "";
		case BRHAL_SPI_FAIL:
			return "BRHAL: SPI Failure\n";
		case BRHAL_GPIO_FAIL:
			return "BRHAL: GPIO Failure\n";
		case BRHAL_TIMER_FAIL:
			return "BRHAL:Timing Function Failure\n";
		case BRHAL_WAIT_TIMEOUT:
			return "BRHAL:HAL Access Timed out\n";
		case BRHAL_GEN_SW:
			return "BRHAL:HAL Function Soft Error\n";
		case BRHAL_WARNING:
			return "BRHAL:HAL Function Soft Warning\n";

		default:
			return "Invalid BRHAL error passed, not in error list\n";
		}
		...
		case TAL_ERR_INV_NULL_INIT_PARAM:
			return "Br3109 init data structure pointer is null\n";
		...
		case TAL_ERR_SETTXATTEN_INV_STEPSIZE_PARM:
			return "Invalid TxAtten stepsize. Use enum br3109TxAttenStepSize_t\n";
		...
		case TAL_ERR_PROGRAMFIR_INV_FIRNAME_PARM:
			return "Invalid FIR filter name requested. use enum br3109firName_t\n";
		...
		case TAL_ERR_RXFRAMER_INV_PCLKFREQ:
			return "Could not find a valid PCLK divider setting when setting up the Br3109 framers\n";
		...
		case TAL_ERR_VERPFILE_INV_RFPLLMCSMODE:
			return "Br3109 init->clocks.rfPllPhaseSyncMode structure member has invalid option for RFPLL phase sync mode\n";
		...
		case TAL_ERR_DAC_FULLSCALE_INVARMSTATE:
			return "TALISE_setDacFullScale() can only be called before Br3109 ARM is loaded\n";
		...
		case TAL_ERR_SETHD2CFG_ARMSTATE_ERROR:
			return "TALISE_setRxHd2Config: Br3109 ARM can not be in radioOn state when the function is called.\n";
		...
		case TAL_ERR_GETHD2CFG_ARMSTATE_ERROR:
			return "TALISE_getRxHd2Config: Br3109 ARM can not be in radioOn state when the function is called.\n";
		...
		case TAL_ERR_INV_SIREV:
			return "Br3109 API only supports Br3109 B1 silicon and greater.  Older Si detected.\n";
		case TAL_ERR_REGISTER_ERRORMSG_C0:
			return "talRegisterErrorMessage() failed for Br3109.\n";
...
const char* TALISE_getRegisteredErrorMessage(br3109Device_t *device,
		uint32_t errSrc, uint32_t errCode)
{
#ifndef TALISE_VERBOSE
	return "";
...
talRecoveryActions_t talApiErrHandler(br3109Device_t *device, br3109ErrHdls_t errHdl,
		uint32_t detErr, talRecoveryActions_t retVal, talRecoveryActions_t recAction)
{
	switch (errHdl)
	{
		...
		case TAL_ERRHDL_HAL_WAIT:
			if (detErr != BRHAL_OK)
			{
				brWriteToLog(device->devHalInfo, BRHAL_LOG_ERR, detErr,
					   TALISE_getErrorMessage(TAL_ERRSRC_BRHAL, detErr));
				device->devStateInfo.errSource = TAL_ERRSRC_BRHAL;
