/**
 * \file br3109_agc_types.h
 * \brief Contains Br3109 API AGC data types
 *
...
#ifndef BR3109_AGC_TYPES_H_
#define BR3109_AGC_TYPES_H_
...
typedef struct {
	...
	uint16_t hb2HighThresh;                      /*!< AGC HB2 output high threshold. Valid range from  0 to 2^14 */
	uint16_t hb2UnderRangeLowThresh;             /*!< AGC HB2 output low threshold. Valid range from  0 to 2^14 */
	uint16_t hb2UnderRangeMidThresh;             /*!< AGC HB2 output low threshold for 2nd interval for multiple time constant AGC mode. Valid range from  0 to 2^14 */
	uint16_t hb2UnderRangeHighThresh;            /*!< AGC HB2 output low threshold for 3rd interval for multiple time constant AGC mode. Valid range from  0 to 2^14 */
...
typedef struct {
	...
	uint8_t powerLogShift;                          /*!< Enable Increase in dynamic range of the power measurement from 40dB to ~60dB. Provides higher accuracy.
                                                         *   NOTE: If you input a signal below -60dBFS while enabled, BR3109_getRxDecPower() may return an incorrect reading of 0dBFS.
                                                         *   When disabled, BR3109_getRxDecPower() will bottom out around -40dBFS. */
...
typedef struct {
	...
	uint8_t agcEnableIp3OptimizationThresh;     /*!< (API disables feature, this member is ignored) Enable the two-threshold AGC loop mode.To improve IIP3. Enable=1, Disable=0 (set to 0 for Br3109 B1 and C0 silicon - this feature is not supported) */
	...
	br3109AgcPower_t agcPower;
	br3109AgcPeak_t agcPeak;

} br3109AgcCfg_t;

/**
 * \brief Data structure to hold all AGC Dualband configuration.
 * Used along with br3109RxChannels_t structure to setup AGC for Rx1 and/or Rx2
 */
typedef struct {
	...
	uint8_t dualBandGpioEnable;                 /*!< Enable the 3.3V GPIO's that would be used to drive the external LNA's.
                                                     ...
                                                     If disabled, user needs to use BR3109_getDualBandLnaControls() to read back the LNA control
...
} br3109AgcDualBandCfg_t;

/**
 * \brief Data structure to hold AGC Dualband LNA controls for the external LNA's.
 * Used along with br3109RxChannels_t structure to read back LNA controls for Rx1 or Rx2
 */
typedef struct {
...
} br3109DualBandLnaControls_t;

#ifdef __cplusplus
}
#endif

#endif /* BR3109_AGC_TYPES_H_ */
