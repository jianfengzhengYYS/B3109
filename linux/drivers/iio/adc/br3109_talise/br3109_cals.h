/**
 * \file br3109_cals.h
 * \brief Contains Br3109 calibration related function prototypes for
 *        br3109_cals.c
 *
...
#ifndef BR3109_CALS_H_
#define BR3109_CALS_H_
...
/**
 * \brief Runs the Br3109 initialization calibrations
 *
 ...
 *        the frequency scan range needs to be set via BR3109_SetFhmConfig() API
 *
 * \dep_begin
 ...
 * \param calMask A bitmask that informs the Br3109 ARM processor which calibrations to run
 *
...
/**
 * \brief Blocking waits for the Br3109 initialization calibrations to complete
 *
 * \pre This function is called after BR3109_runInitCals to wait for them to finish
 *
 * \dep_begin
 * \dep{device->devHalInfo}
 * \dep_end
 ...
 *                  Error flag enumerated by br3109InitCalFlags_t. A value of 0 indicates that there
 *                  was no error.
 *
 * \retval TALACT_WARN_RESET_LOG Recovery action for log reset
 * \retval TALACT_ERR_CHECK_PARAM Recovery action for bad parameter check
 ...
 *
 * This function is similar to BR3109_waitInitCals except it does
 ...
 * BR3109_runInitCals() function call
 *
 * \retval TALACT_WARN_RESET_LOG Recovery action for log reset
 * \retval TALACT_ERR_CHECK_PARAM Recovery action for bad parameter check
 * \retval TALACT_ERR_CHECK_TIMER Recovery action for timer time-out check required
 ...
 * Each bit represents a calibration. br3109InitCalibrations_t defines the unique initialization
 ...
 * calibration set to run during the radio ON state. br3109TrackingCalibrations_t
 ...
 */
  ...
  * br3109TrackingCalibrations_t enumerated types are or'd together to form the
  ...
  *        of type br3109TxLolStatus_t is returned to this pointer address
  *
  * \retval TALACT_WARN_RESET_LOG Recovery action for log reset
  * \retval TALACT_ERR_CHECK_PARAM Recovery action for bad parameter check
  * \retval TALACT_ERR_RESET_SPI Recovery action for SPI reset required
  ...
  *        of type br3109TxQecStatus_t is returned to this pointer address
  *
  * \retval TALACT_WARN_RESET_LOG Recovery action for log reset
  * \retval TALACT_ERR_CHECK_PARAM Recovery action for bad parameter check
  * \retval TALACT_ERR_RESET_SPI Recovery action for SPI reset required
  ...
  *        of type br3109RxQecStatus_t is returned to this pointer address
  *
  * \retval TALACT_WARN_RESET_LOG Recovery action for log reset
  * \retval TALACT_ERR_CHECK_PARAM Recovery action for bad parameter check
  * \retval TALACT_ERR_RESET_SPI Recovery action for SPI reset required
  ...
  *        of type br3109OrxQecStatus_t is returned to this pointer address
  *
  * \retval TALACT_WARN_RESET_LOG Recovery action for log reset
  * \retval TALACT_ERR_CHECK_PARAM Recovery action for bad parameter check
  * \retval TALACT_ERR_RESET_SPI Recovery action for SPI reset required
 ...
 *        of type br3109RxHd2Status_t is returned to this pointer address
 *
 * \retval TALACT_WARN_RESET_LOG Recovery action for log reset
 * \retval TALACT_ERR_CHECK_PARAM Recovery action for bad parameter check
 * \retval TALACT_ERR_RESET_SPI Recovery action for SPI reset required
 ...
 */
 ...
 /**
  * \brief Performs a blocking wait for a Br3109 calibration or Pll Lock
  *
  * \dep_begin
  * \dep{device->devHalInfo}
  * \dep_end
 ...
 /**
  * \brief Performs a non-blocking read of wait status for a Br3109 calibration or Pll Lock
  *
  * \dep_begin
  * \dep{device->devHalInfo}
  * \dep_end
...
/**
  * \brief Allows setting configuration options for Br3109 Rx HD2 tracking calibration
  *
  ...
  *
  * \pre This function can only be called when the Br3109 ARM is in the Radio Off state (or
...
/**
  * \brief Allows reading configuration options for Br3109 Rx HD2 tracking calibration from
  * Br3109 ARM memory
  *
  * Config values are read from the Br3109 ARM memory and returned at the address of
...
/**
  ...
  * Br3109 ARM init cals have been run. For 4.0 and greater, the function
 ...
 *  The mask can be a combination of the following channel values ( ::br3109RxDcOffsettEn_t ).
 *
 ...
 *
 * \retval BR3109_ERR_DIG_DC_OFFSET_INV_ENABLE_MASK enable mask passed to the function is invalid, refer br3109RxDcOffsettEn_t enum.
 * \retval BR3109_ERR_OK Function completed successfully
 */
 ...
 * The mask returned will be a combination of the following channel values ( ::br3109RxDcOffsettEn_t ).
 ...
 *
 * \retval BR3109_ERR_DIG_DC_OFFSET_NULL_ENABLE_MASK enableMask is NULL
 * \retval BR3109_ERR_OK Function completed successfully
 */
uint32_t BR3109_getDigDcOffsetEn(br3109Device_t *device, uint8_t *enableMask);

    /**
      * \brief Allows reading Br3109 tracking calibration batch size configuration option
      * from Br3109 ARM memory
      *
      * Config value is read from the Br3109 ARM memory and returned at the address of
      * the batchsize_us enum function parameter.
      *
      * \pre This function can only be called when the Br3109 ARM is in the RadioOff/IDLE state or ready state.
      *
      * \dep_begin
      * \dep{device->devHalInfo}
      * \dep_end
